...
&nbsp;
&nbsp;&nbsp;&nbsp;`-api` 모듈을 따로 뗀 이유는 Axon이 메시지(Command·Event) 기반으로 동작하기 때문이다. 이벤트를 발행하는 쪽, 소비하는 쪽, 어드민까지 여러부분에서 공유한다. 무거운 구현 모듈 전체에 의존하게 두면 결합도가 올라가서, 별도로 정의만 담은 가벼운 `-api` 모듈을 두고 의존하는 쪽은 이 라이브러리만 바라보게 했다. 실행 대상이 아니라 `bootJar`는 끄고 일반 `jar`만 만들었고, `api(project(":common"))`로 공통 정의를 전이(transitive) 노출했다. 모듈 경계를 이렇게 잡아 두니 의존 관계가 빌드 설정만 봐도 드러났다.

&nbsp;&nbsp;&nbsp;프로젝트 셋업을 마치고 설계에서 도출한 이벤트와 Aggregate를 실제 코드로 옮기기 시작했다. 프레임워크는 [Axon](https://www.axoniq.io/)을 선택했다. 프레임워크를 선택한 가장 큰 이유는 이벤트 소싱을 직접 구현하는 부담이었다. 이벤트 저장·재생, Command/Event/Query 분리, Projection 같은 플로우를 처음부터 직접 작성하는 것이 결국 생산성에 발목을 잡을것이라 생각했기 때문이다. 팀 전원이 Kotlin·Spring도 이벤트 소싱도 처음이었던 만큼, 러닝커브가 있더라도 검증된 프레임워크를 공부하며 빠르게 적용해보는 편이 낫다고 판단했다.

&nbsp;&nbsp;&nbsp;실제로 Axon은 필요한 기능을 폭넓게 제공했다. Aggregate·Command/Event/Query 버스·Event Store는 물론이고, 이벤트 리플레이, Saga, DLQ, AxonIQ Console 모니터링까지 운영 기능 상당수를 프레임워크 안에서 이미 기능으로 제공하고 있었다.

&nbsp;&nbsp;&nbsp;전체 코드 흐름을 보면 아래와 같다.

1. Inbound (REST or Kafka)
2. Service (UseCase)
3. Command (Handler)
4. Event store (AxonServer)
5. Aggregate Create, Read, Update
6. Outbound (Projection)(비동기)

&nbsp;
&nbsp;&nbsp;&nbsp;API나 Kafka를 통해 들어온 요청을 Service Layer에서 Command로 생성하고, 해당 Command는 검증후 도메인 Layer의 Aggregate로 전달된다. Aggregate는 로직을 처리한 뒤 결과를 이벤트로 발행해 Event Store에 저장하며, 이때 해당 이벤트가 Aggregate 자신의 상태도 갱신한다. 저장된 이벤트는 이를 구독하는 Projection이 받아 Read Model을 갱신(비동기)하는 구조다.

### 레거시와의 공존

&nbsp;&nbsp;&nbsp;가장 현실적인 제약은 레거시 Django를 당장 내릴 수 없다는 점이었다. 그래서 마이그레이션 기간 동안 두 시스템이 동시에 돌아가는 구조를 전제로 잡았다. 레거시에서 발생한 도메인 변경을 Kafka로 흘려보내면, 새로운 시스템에 반영하는 방향으로 잡았다.

&nbsp;&nbsp;&nbsp;여기서 세 가지를 고민해야 했다.

1. 이벤트 발행 — 정석대로라면 Outbox 테이블로 비즈니스 트랜잭션과 메시지 적재를 한 트랜잭션에 묶는 방법이 있다. 실제로 프로토타입까지 만들어 테스트해봤지만, 우리 규모에서는 과하다고 판단했다. 최종적으로는 Django signal로 도메인 변경 시점에 Kafka로 발행하는 단순한 방식을 택했다.
2. 정합성 검증 — Django Signal 방식으로 작업하더라도 데이터가 누락할수 있는 가능성 충분히 있었다. 그래서 이를 보완하기위해 레거시에 API relay를 두어, 해당 API가 호출되면 레거시 데이터와 새 시스템의 ES Projection을 비교·검증하고 불일치를 잡아냈다.
3. 기존 데이터 백필 — 실시간 변경분과 별개로, 이미 쌓여 있던 과거 데이터를 옮겨야 했다. 이를 위해 일반 Command와 구분되는 `Migrate*` 커맨드/이벤트를 따로 두었다. 마이그레이션 이벤트는 일반 이벤트와는 다르게, 대량 적재가 필요하기 때문에 불필요한 validation 로직을 제거하고 batch 처리를 통해 빠르게 처리되도록 하여, 일반 이벤트와 구분하였다.
...
&nbsp;&nbsp;&nbsp;레거시가 보내는 JSON에는 같은 필드가 어떤 때는 배열로, 어떤 때는 단일 값으로 담겨 오는 경우가 있었다. 원소가 하나뿐이면 배열이 아니라 값 하나로 직렬화되는 식이다. Python 쪽에서는 타입을 강제하지 않다 보니 이런 느슨함이 자연스럽게 섞여 들어왔는데, 받는 Kotlin은 `List<T>`로 딱 정해져 있어서 단일 값이 오면 파싱이 깨졌다.

&nbsp;&nbsp;&nbsp;`ACCEPT_SINGLE_VALUE_AS_ARRAY` 옵션으로 흡수했다. 단일 값이 와도 원소 하나짜리 배열로 감싸 받아들이게 하는 설정이다. 결국 레거시의 느슨한 직렬화와 새 시스템의 엄격한 타입 사이의 간극을 메우는 작업이었다.

**④ 커스텀 Deserializer**

&nbsp;&nbsp;&nbsp;레거시가 담아 보내는 값이 새 모델의 타입과 1:1로 맞지 않는 경우도 많았다. 대표적으로 enum이 그랬다. 레거시는 급여 유형을 `"H"`, `"D"` 같은 짧은 코드로 보내기도 하고 `"HOURLY"`, `"DAILY"` 같은 전체 이름으로 보내기도 했는데, Kotlin enum의 기본 `valueOf`는 `"H"`를 만나면 그대로 예외를 던진다. 그래서 두 표기를 모두 받아 하나의 enum으로 매핑하는 전용 Deserializer를 만들었다.

```kotlin
class PayTypeDeserializer : JsonDeserializer<PayType>() {
    override fun deserialize(p: JsonParser, ctxt: DeserializationContext): PayType =
        when (val value = p.text.trim()) {
            "H", "HOURLY" -> PayType.HOURLY
            "D", "DAILY"  -> PayType.DAILY
            "W", "WEEKLY" -> PayType.WEEKLY
            // ...
            else -> PayType.valueOf(value)
        }
}
```

&nbsp;&nbsp;&nbsp;시간 표현도 마찬가지였다. 레거시가 보내는 날짜 문자열을 그대로 파싱하면 타임존 기준이 어긋나서, `OffsetDateTimeDeserializer`로 파싱 시점에 기준 시간대로 정규화했다(타임존 처리는 그 자체로 계속 발목을 잡은 성가신 주제였다). 결국 ④는 Django가 느슨하게 흘려보낸 값들을 새 시스템의 타입 체계 안으로 하나씩 번역해 들이는 작업이었다.
