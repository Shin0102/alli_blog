...
&nbsp;&nbsp;&nbsp;기존 정산시스템의 경우 사용자에게 할당된 가상계좌로 입금이 되면 뱅킹시스템으로부터 API Callback을 받아 정산할 금액을 계산하고 이를 외부 정산 시스템에 전달(HTTP 통신)하는 구조였다. 레거시로 인해 불필요하게 시스템들이 나누어져 있었고 각 단계마다 의존성이 커 중간에 실패가 발생할 경우 이슈파악이나 처리가 어려웠다. 사용자가 늘어 나면서 급여일에는 처리누락이 다수 발생하였고, 이를 모니터링하기 위해 급여일에는 2~3명의 인원이 필요했다. 이로인해 인력 리소스 낭비가 심한 상태였다. 
...
&nbsp;&nbsp;&nbsp;하지만 한가지 주의해야할 점이 있었는데, 이벤트간의 순서였다. 정산처리를 위해선 몇번의 이체단계가 필요한데, 이벤트간의 순서가 지켜지지 않으면 이체가 정상적으로 동작하지 않기 때문이다. 이를 해결하기 위해서 ***Zero-Payload 전략과 단계별 이벤트 체이닝***을 차용하였다. 간단히 설명하면 이벤트 처리가 완료되면 다음 이벤트 처리를 위한 data를  db에 저장함과 동시에 큐에 이벤트 id를 Publish 하였다. 이벤트를 Consume 하는곳에서 Publish된 이벤트 id 정보를 받아 이벤트에 해당하는 처리를 하고 db에 성공유무를 업데이트 하도록 하였다.

{{< img src="/images/event-driven-architecture/new.png" caption="event 처리 flow" width="500px" position="center" >}}

&nbsp;&nbsp;&nbsp;여기서 한 가지 더 고민한 부분은 ***Zero-Payload 방식의 정합성과 순서 보장*** 문제였다. 이벤트 본문에 데이터를 싣지 않고 이벤트 id만 publish 하기 때문에, 이벤트 처리 순서가 어긋나거나 동일 이벤트가 동시에 처리되면 이체가 꼬일 수 있었다. 이를 막기 위해 ***이벤트 읽기(SQS read)와 처리 태스크 호출을 하나의 단일 태스크로 묶고, Redis Lock으로 직렬 실행을 강제***했다. 즉 락을 획득한 태스크만 이벤트를 읽어 처리 태스크를 호출하므로, 같은 정산 건의 이벤트가 동시에 중복 처리되는 상황 자체를 차단했다.

&nbsp;&nbsp;&nbsp;그리고 처리 태스크가 ***성공적으로 완료되었을 때에만 다음 단계의 이벤트를 생성***하도록 했다. 이렇게 단계 간 이벤트 생성을 처리 성공에 종속시켜, 이체 A가 끝나야 이체 B 이벤트가 만들어지는 ***순차 보장***을 얻을 수 있었다. 추가로 Zero-Payload 특성상 Consumer가 이벤트 id로 DB를 조회하는 시점에 "아직 커밋되지 않은 데이터"를 읽지 않도록, ***데이터를 DB에 커밋한 뒤에 publish***하는 순서도 함께 강제했다.

&nbsp;&nbsp;&nbsp;정리하면 (1) Zero-Payload로 메시지 크기와 큐 의존성을 줄이고, (2) Redis Lock 단일 태스크로 동시 처리를 막고, (3) 처리 성공 시에만 다음 이벤트를 생성해 단계 순서와 중복 처리를 함께 차단하는, ***다중 안전장치***를 갖추게 되었다.

{{< boxmd >}}
- Redis Lock 단일 태스크 → 같은 정산 건의 이벤트가 동시에 중복 처리되는 상황을 차단
- 처리 태스크가 성공해야 다음 단계 이벤트가 생성 → 단계 순서 보장과 중복 이체 방지를 동시에 해결
- 데이터 DB 커밋 후 publish → Zero-Payload 조회 시 미커밋 데이터 read 방지
- 미처리 건 조회 배치 + 외부 API 멱등성으로 누락·중복을 이중 방어
{{< /boxmd >}}

&nbsp;&nbsp;&nbsp;다만 비동기 + 재시도 구조에서는 워커 장애나 메시지 재전달로 인한 ***중복 처리와 처리 누락***을 항상 염두에 둬야 했다. 중복 이체의 경우, 이체 처리에 대해서는 ***외부 뱅킹 API가 자체적으로 중복 방지***를 제공하고 있었기 때문에 이를 적극 활용하여 동일 이체가 두 번 실행되는 상황을 차단했다. 또한 어떤 이유로든 다음 단계로 진행되지 못하고 멈춘 건을 잡아내기 위해, ***미처리 건을 주기적으로 조회하는 별도 태스크***를 두어 일정 시간 이상 처리되지 않은 정산은 자동으로 재시도되도록 했다. 이로써 "한 번은 반드시, 그러나 두 번은 절대 처리되지 않는다"는 정산의 핵심 요구사항을 만족시킬 수 있었다.
...
&nbsp;&nbsp;&nbsp;다행히도 성공적으로 마이그레이션을 마무리했고, 회사뿐만 아니라 개인적으로도 큰 임팩트가 있는 프로젝트였다.

&nbsp;&nbsp;&nbsp;돌아보면 아쉬운 점도 분명했다. 미처리 건 재시도나 장애 알림 같은 ***런타임 복구 장치***는 갖춰 두었지만, 혼자 빠르게 진행하다 보니 ***테스트 자동화(검증 커버리지)가 부족***했다. 핵심 이체 로직과 이벤트 흐름에 대한 자동화된 테스트가 충분치 않아, 배포 전 검증을 상당 부분 수기 확인에 의존해야 했다. 다음에 비슷한 시스템을 설계한다면 ***검증 자동화와 관측성(이벤트 추적 대시보드)을 1순위***로 두고 시작할 것이다.