...
&nbsp;&nbsp;&nbsp;여기서 한 가지 더 고민한 부분은 ***Zero-Payload 방식의 정합성과 순서 보장*** 문제였다. 이벤트 본문에 데이터를 싣지 않고 이벤트 id만 publish 하기 때문에, 이벤트 처리 순서가 어긋나거나 동일 이벤트가 동시에 처리되면 이체가 꼬일 수 있었다. 이를 막기 위해 ***이벤트 읽기(SQS read)와 처리 태스크 호출을 하나의 단일 태스크로 묶고, Redis Lock으로 단일 실행을 강제***했다. 즉 락을 획득한 태스크만 이벤트를 읽어 처리 태스크를 호출하므로, 같은 정산 건의 이벤트가 동시에 중복 처리되는 상황 자체를 차단했다.

&nbsp;&nbsp;&nbsp;그리고 처리 태스크가 ***성공적으로 완료되었을 때에만 다음 단계의 이벤트를 생성***하도록 했다. 이렇게 단계 간 이벤트 생성을 처리 성공에 종속시켜, 이체 A가 끝나야 이체 B 이벤트가 만들어지는 ***순차 보장***을 얻을 수 있었다.

&nbsp;&nbsp;&nbsp;정리하면 (1) Zero-Payload로 메시지 크기와 큐 의존성을 줄이고, (2) Redis Lock 단일 태스크로 동시 처리를 막고, (3) 처리 성공 시에만 다음 이벤트를 생성해 단계 순서와 중복 처리를 함께 차단하는, ***안전장치***를 갖추게 되었다.
...
&nbsp;&nbsp;&nbsp;마이그레이션 이후, 급여일에도 정산처리가 지연되거나 실패되는 이슈가 ***제로에 수렴***했다. 외부적인 요소(계좌정지, 예금주명 변경 등등)에 의해서 이체나 정산이 실패하는 경우에는 슬랙 알림과 함께 ***재시도 로직을 수행***하여 최대한 빠르게 정산이 이루어지도록 하였으며, 또한 레거시 외부시스템들의 의존성을 제거함으로써 ***매월 수백만원의 지출 감소효과***도 얻게 되었다. 
