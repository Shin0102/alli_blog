...
&nbsp;&nbsp;&nbsp;백오피스에 보여지는 데이터들은 비교적 변경이 적고 실시간성이 크게 중요하지 않았기 때문에 위와 같은 결정을 할수있었다. (그렇다고 delay가 엄청긴것은 아니였으며, 현재 데이터 변경부터 NoSQL DB 저장까지 1분 이내에 반영되고 있다.)

&nbsp;&nbsp;&nbsp;한가지 더 짚고 넘어갈 부분은, 하나의 문서(Document)가 여러 테이블을 역정규화해 합친 형태라는 점이다. 그런데 CDC는 **테이블 단위의 변경분**만 전송되기 때문에, 매 이벤트마다 전체 문서를 새로 만들 수는 없었다. 그래서 Lambda는 변경된 테이블에 해당하는 **부분 필드만 기존 문서에 갱신**(`$set`)하는 방식으로 동작하도록 했다. 각 테이블의 CDC가 도착할 때마다 자기 영역의 필드만 채워 넣어, 데이터가 완성되는 구조다. 변경분 처리 시 MySQL을 다시 조회할 필요가 없었고, 서로 다른 테이블의 이벤트는 '**다른 필드를 건드리므로 테이블 간 도착 순서가 뒤섞여도 서로를 덮어쓰지 않는다**'는 이점도 있었다.

## TroubleShooting

---

&nbsp;&nbsp;&nbsp;이번엔 위에 같은 컨셉을 가지고 PoC부터 실제 실서버에 적용하기까지 겪었던 TroubleShooting을 정리해보도록 하겠다.
...
&nbsp;&nbsp;&nbsp;실제 프로덕션 전환 시 한가지 주의할 점은, Full Load와 CDC를 따로 돌리면 그 사이에 발생한 변경 데이터가 누락될 수 있다는 것이다. 이를 구조적으로 막기 위해 DMS의 **`full-load-and-cdc` 모드**를 사용하였다. 이 모드는 Full Load 시작 시점부터의 변경분을 버퍼링했다가 Full Load 완료 후 이어서 적용하므로, 변경 유실을 방지한다. 여기에 더해 실제 전환은 트래픽이 가장 적은 새벽 시간대에 진행하였고, 전환 후 **MySQL과 DocumentDB를 비교하는 체크 스크립트**를 주기적으로 실행하고 모니터링하여, 누락 여부까지 직접 검증한 뒤 실제 서비스에 연결하였다.

&nbsp;&nbsp;&nbsp;마지막으로, 설계상 놓쳤던 부분인데 CDC 데이터를 S3에 적재하고 이를 Lambda로 트리거 하도록 하는것은 좋지 못한 선택이었다. 위에서도 언급했듯이, Source Endpoint가 RDS인 경우 **데이터를 로드하는 순서가 중요한데 해당 플로우는 로드순서를 보장해주지 못한다**.('load-order'는 Full Load시에만 적용되는 옵션이었음…) 그래서 업데이트 실패 시 **지수 백오프 방식의 재시도로직을 추가해서 대응**하였고 정상적으로 동작함을 확인하였다.(물론 재시도로직 때문에 추가적인 delay가 발생하는것은 피할수 없었다) 이 부분은 추후 **데이터 스트림이나 Queue 시스템을 활용**하도록 변경할 예정이다.

&nbsp;&nbsp;&nbsp;정합성 측면에서 다행이었던 점은, Lambda에서 소스 PK를 기준으로 **upsert**로 데이터를 반영했기 때문에 동일한 CDC 이벤트가 중복 전달되더라도 결과가 달라지지 않는 **멱등성**은 확보되어 있었다는 것이다. 또한 앞서 설명한 부분 필드 갱신 방식 덕분에, 서로 다른 테이블의 이벤트는 다른 필드를 건드려 테이블 간 순서 문제는 크게 완화되었다. 한가지 놓쳤던 부분은 **동일 row에 대한 여러 변경이 뒤바뀌어 도착**하는 경우였다. 이때는 옛 데이터가 최신 데이터를 덮어쓸 수 있었고 이를 재시도에 의존해 대응했다. 이는 추후에 각 CDC 레코드의 commit timestamp를 문서에 저장해두고 **들어온 레코드가 더 최신일 때만 갱신하는 조건부 upsert**를 적용하여 정합성을 보장하도록 대응하였다.
...
&nbsp;&nbsp;&nbsp;설계 측면에서 아쉬웠던 부분을 다시 작업한다면 다음과 같이 개선하고 싶다. `S3 → Lambda` 직접 트리거 대신 **`S3 → SQS → Lambda` 구조와 DLQ**(Dead Letter Queue)를 두어 이벤트 유실과 실패 메시지를 안전하게 격리·재처리한게끔 하여 오류에 좀 더 유연한 시스템이 되도록 개선해보고 싶다.

&nbsp;&nbsp;&nbsp;현재 서비스가 성장하면서 비즈니스 로직이 점점 복잡해지고 성능과 코드 가독성 개선에 한계를 점점 느끼고 있었는데, 이번에 새롭게 **Amazon DocumentDB**(MongoDB 호환)를 도입함으로써, 앞으로 문제 해결을 위해 DocumentDB를 적극적으로 활용할 수 있을 것 같다.